...
From the code below we set a default data folder we run the code and list the data files in the folder.
For our case, it's
...
file_path <- "../../data/coinbaseUSD_1-min_data_2014-12-01_to_2018-11-11.csv"

# Read the CSV file into a data frame named 'data'
data <- read_csv("../../data/coinbaseUSD_1-min_data_2014-12-01_to_2018-11-11.csv")
...
Although the plots interactively match that of R programming, we see a superiority of the R language in handling the visualizations in a more versatile manner by creating multiple facets for each column while still maintaining the same x and y axis.
...
This code functions exactly as the previous one.
The plot shows a spike in prices every October - December for each year.
...
```{r, message=FALSE}
data <- read_csv("../../data/bitstampUSD_1-min_data_2012-01-01_to_2018-11-11.csv")  # Read CSV data into 'data' dataframe
data <- data %>% 
